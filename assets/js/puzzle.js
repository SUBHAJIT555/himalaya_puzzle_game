let puzzle, autoStart, timerInterval;
...
function startTimer() {
  ...
  timerInterval = setInterval(() => {
    ...
    if (timeLeft <= 0) {
      clearInterval(timerInterval);
      const failureMessage = encodeURIComponent("Loss");
      window.location.href = `result.html?message=${failureMessage}`;
    }
  }, 1000);
}
...
function loadInitialFile() {
  ...
  // Set the selected image as the source for the puzzle
  puzzle.srcImage.src = selectedImage;
  eyeModal(selectedImage);
...

{
  ...
  animate = function () {
    ...
    break;
      ...
      case 60: // Winning
        const successMessage = encodeURIComponent("Won");
        window.location.href = `result.html?message=${successMessage}`;
        break;
